...
<div class="app-body">
    ...
    <div id="results">
        <?php
            $conn = mysqli_connect('localhost','root','','rto');
            $sql = "SELECT * FROM drivingschool";
            $result = $conn->query($sql);

            if($result->num_rows > 0){
                while($row = $result->fetch_assoc()) {
        ?>
            <div class="question-card" data-city="<?php echo strtolower($row['city']); ?>">
                <div>
                    <table>
                        <tr onclick="openMap('<?php echo $row['name']; ?>');">
                            <td>
                                <span id='question'><?php echo $row['name']; ?></span>
                            </td>
                            <td></td>
                        </tr>
                        <tr>
                            <td>
                                <i class="fa fa-phone"></i>&nbsp;&nbsp;<label><?php echo $row['mobile']; ?></label>
                            </td>
                            <td></td>
                        </tr>
                        <tr onclick="openMap('<?php echo $row['name']; ?>');">
